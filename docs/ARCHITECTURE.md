...
Jedes aktive Familienmitglied behält aus Gründen der Nachvollziehbarkeit eine eigene SQLite-Zeile und eine eigene Tischzuweisung. Das Feld `root_booking_id` verbindet die Zeilen. Die Hauptbuchung wird jedoch zuerst und genau einmal mit der zusammengeführten Liste aller Tische gepatcht. So enthält die kundenrelevante Bestätigung beispielsweise `Deine Tische: Tisch 1, Tisch 2, Tisch 3`. Die technischen Unterbuchungen erhalten anschließend ihre jeweilige Teilzuweisung und einen kompakten Admin-Titel wie `TISCHE: Tisch 2 — Zusatz zu #BB123456789`. Dadurch bleiben die von Anny separat dargestellten Datensätze eindeutig der Hauptbuchung zugeordnet, ohne deren vollständigen ursprünglichen Titel zu wiederholen. Ein Ereignis einer Unterbuchung aktualisiert ebenfalls die Hauptbuchung. Wird eine Unterbuchung entfernt, verschwindet ihr Tisch aus der zusammengeführten Liste und die frei gewordene Kapazität kann nachverteilt werden.
